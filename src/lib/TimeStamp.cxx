...
bool TimeStamp::isEqual(const AbsObject& other) const
{
...
/** Compare this to other. 
    ...
    -  0 else
*/
int TimeStamp::compare(const AbsObject& other) const
{
...
string TimeStamp::asString(bool includeOffset) const
{
 ...
 */
int TimeStamp::tzSecondsOffset() const
{
  string tzo = tzOffset();
 ...
 */
double TimeStamp::difference(const TimeStamp& other) const
{
...
/** Array of days in each month of the year. 
    ...
    \endcode
*/
int* TimeStamp::DaysInMonths(bool isLeapYear)
{
  static int daysInMonth[] = { 31, 0, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
  daysInMonth[1] = isLeapYear ? 29 : 28;
  return daysInMonth;
}
...
{
  // get days in months
  int* dim = DaysInMonths(IsLeapYear(timeInfo.tm_year));

  // copy input struct so that input doesn't change
  tm_t tm_cp = timeInfo;

  // reference values
  int ref_tm_mon = tm_cp.tm_mon;
  int ref_tm_mday = tm_cp.tm_mday;
...
*/
tm_t TimeStamp::GetTM(int year, int month, int day, int hour, int min, int sec)
{
  tm_t timeInfo;
...
*/
bool TimeStamp::ParseDateTime(string dateTime, tm_t& timeInfo)
{
  // check length
...
*/
int TimeStamp::ParseTzOffset(string tzoffset)
{
  // check length
  ...
  int hour = atoi( tzoffset.substr(1,2).c_str() );
  int min  = atoi( tzoffset.substr(3,2).c_str() );

  // convert to seconds
  int sec = int(TimeStamp::HourToSec(hour) + TimeStamp::MinToSec(min));
...
time_t TimeStamp::ParseIsoString(string isoString)
{
  ...
  tm_t timeInfo;
  bool dtFailed = ParseDateTime(isoString, timeInfo);
